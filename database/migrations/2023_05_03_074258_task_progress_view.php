...
return new class extends Migration
{
    ...
    public function up(): void
    {
        DB::statement("CREATE VIEW task_progress_view 
                AS SELECT `brands`.id, `plan_libraries`.id 
                As plan_id, `plan_libraries`.approved_on 
                As plan_approved_on,`design_libraries`.id 
                AS design_id ,`design_libraries`.status 
                AS design_status,`design_libraries`.approved_on 
                AS design_approved_date
                FROM `brands`
